...
const adapter: MdxParserAdapter = {
  inspect: async () => ({ header: { Title: 'fixture' }, mdxVersion: '2.0', encoding: 'UTF-8', entryCount: 3 }),
  async *iterateEntries() {
    yield { headword: 'apple', rawEntry: '<span>apple <b>苹果</b></span>', redirectTarget: null };
    yield { headword: 'audio', rawEntry: '<a href="sound://audio/日本語/東京.ogg"><img src="icon.png"></a>', redirectTarget: null };
    yield { headword: 'apples', rawEntry: '@@@LINK=apple\0', redirectTarget: 'apple' };
  },
};
describe('MDX importer → PostgreSQL', () => {
  ...
  it('persists normalized entries, HTML, plain text, and redirects', async () => {
    ...
    expect(rows).toHaveLength(3); expect(rows[0].headwordNormalized).toBe('apple'); expect(rows[0].entryPlainText).toBe('apple 苹果');
    expect(rows[1].entrySanitizedHtml).toBe('<span data-mdict-kind="sound" data-mdict-resource="audio/日本語/東京.ogg"></span>');
    expect(rows[1].entryPlainText).toBe('');
    expect(rows[2].entryKind).toBe('redirect'); expect(rows[2].redirectTargetOriginal).toBe('apple');
  });
});
